...
import { getUrlParams } from '@/utils/parthSearchUrl';
...
export default () => {
  ...
  const cardID = getUrlParams(search)?.cardId;
  const editCadtName = (id: string) => {
    switch (id) {
      case '1':
        return '第一个项目组组件';
      case '2':
  ...
  // const stateDva = useSelector((newState) => newState.getState().demo.data);

  return (
    <div className={styles.formItemCardBox}>
      {cardID && <div>这是我的图片ID：{editCadtName(cardID)}</div>}
      <Space>
        <Radio.Group
